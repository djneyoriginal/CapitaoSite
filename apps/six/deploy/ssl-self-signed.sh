...
if [ "$(id -u)" -ne 0 ]; then
  echo "Execute como root: sudo bash deploy/ssl-self-signed.sh ${DOMAIN} [IP_DO_SERVIDOR]"
  exit 1
fi
...
if [ -z "${SERVER_IP}" ]; then
  ...
  echo "sudo bash deploy/ssl-self-signed.sh ${DOMAIN} 192.168.0.188"
  exit 1
fi
...
echo
echo "OK. Certificado interno configurado: https://${DOMAIN}"
echo "Aviso: navegadores vao mostrar alerta ate voce instalar/confiar neste certificado nos dispositivos."
echo "Certificado publico para instalar nos clientes: ${CERT_DIR}/six.crt"